...
is downloaded separately under `corpora/`.

The direct Hugging Face route is blocked by the current DNS/proxy path. A local
`hf-cn` wrapper uses the reachable mirror without changing the normal `hf`
command:
